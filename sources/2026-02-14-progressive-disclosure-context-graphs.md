---
title: "Progressive Disclosure & Context Graphs for Knowledge Repos"
created: 2026-02-14
updated: 2026-02-14
template: templates/source.md
template_version: 3
tags: [source, ai-pm, context-engineering, progressive-disclosure, knowledge-graphs, retrieval-architecture]
status: processed
source_type: note
source_url: ""
archive_url: "domains/professional-development/ai-pm/sources/2026-02-14-progressive-disclosure-context-graphs.md"
author: "Geoff Dudgeon, Claude"
published: 2026-02-14
discovered: 2026-02-14
summary: "Research synthesis on progressive disclosure (3-layer retrieval for agent context) and context graphs (temporal knowledge graphs, GraphRAG) for knowledge repos. Surveys Claude-Mem, Letta Context Repositories, Graphiti/Zep, Klavis Strata. Recommends frontmatter summaries, typed entries, filesystem-as-index, agent-managed defragmentation. Assesses home-brain gaps and proposes graduation path from filesystem-only to graph-backed retrieval."
...
*Research synthesis by Geoff Dudgeon & Claude*

## Summary

A comprehensive survey of progressive disclosure and context graph patterns for agent-oriented knowledge repositories. The strongest finding is convergence: multiple independent implementations (Claude-Mem, Letta, Inferable, Strata) have arrived at the same 3-layer retrieval pattern (index → summary → full content) for managing agent context. Letta's Context Repositories validate that filesystem primitives alone constitute a legitimate retrieval architecture — not a stopgap. Context graphs (Graphiti/Zep, GraphRAG) are powerful but premature for most individual/small-team knowledge bases. The most actionable framing is a graduation path: start with filesystem-only conventions that work everywhere, layer on semantic search when scale demands it, and defer graph databases until entity complexity justifies the infrastructure.
...
- [Progressive Tool Disclosure](../knowledge-base/horizontal/agents/skills/progressive-tool-disclosure.md) — Revealing tools in layers to combat MCP tool overload (+15% accuracy)
...
- The document's recommendations about adopting changes in home-brain were evaluated and declined — the current filesystem structure already implements the patterns well, and the metadata additions (summary everywhere, typed entries, related_entities) would create maintenance burden that contradicts the "indexes go stale" lesson
- The Letta Context Repositories blog post (Feb 12, 2026) is the most architecturally relevant reference and would be worth capturing as its own source for deeper processing
- The ClawVault source (already in queue, unread) covers overlapping territory — agent memory modeled on Obsidian, markdown+YAML outperforming specialized memory infra
- Strong connection to existing entries: Filesystem as Agent State (state vs retrieval), Deliberate Context Selection (system-level vs individual practice), Self-Maintaining Knowledge Bases (agent-managed defragmentation)
